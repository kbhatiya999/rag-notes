...
2. Create a new folder named `.litellm` (it will warn you about starting with a dot, confirm it).
   *(Remember: press `⇧ + ⌘ + .` to see hidden folders starting with a dot in Finder)*.
3. Inside the `.litellm` folder, open Zed and create a new blank document.
